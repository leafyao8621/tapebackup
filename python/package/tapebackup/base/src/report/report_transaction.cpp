...
void TBCLI::ReportTransaction::report_text(sqlite3_stmt *stmt, std::ostream &os) {
    int ret = 0;
    for (;;) {
        ...
        os << "ID: " << sqlite3_column_int64(stmt, 0) << std::endl <<
            "Signature: " << sqlite3_column_text(stmt, 1) << std::endl <<
            "Write protection: " <<
            ...
            "Completion time: " <<
            sqlite3_column_text(stmt, 7) << std::endl <<
            "Reported size: " <<
            sqlite3_column_int64(stmt, 8) << std::endl <<
            "Written size: " <<
            sqlite3_column_int64(stmt, 9) << std::endl;
    }
...
void TBCLI::ReportTransaction::report_csv(sqlite3_stmt *stmt, std::ostream &os) {
    int ret = 0;
    os <<
        "\"ID\"," <<
        "\"Signature\"," <<
...
void TBCLI::ReportTransaction::operator()(
