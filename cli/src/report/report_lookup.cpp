...
void TBCLI::ReportLookup::report_text(sqlite3_stmt *stmt, std::ostream &os) {
    int ret = 0;
    for (;;) {
...
void TBCLI::ReportLookup::report_csv(sqlite3_stmt *stmt, std::ostream &os) {
    int ret = 0;
    os <<
        "\"Signature\"," <<
        "\"Write Protection\"," <<
...
void TBCLI::ReportLookup::operator()(
